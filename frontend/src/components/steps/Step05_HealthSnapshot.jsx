...
export default function Step05_HealthSnapshot({ formData, updateField }) {
    ...
    return (
        <StepWrapper className="space-y-8">
            ...
            <div className="flex justify-around bg-white/5 border border-white/10 p-6 rounded-2xl">
                <div className={`p-3 rounded-full transition-all duration-300 ${formData.smoking_status === 'Never' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-slate-600'}`}>
                    <Cigarette className="w-6 h-6" />
                </div>
                <div className={`p-3 rounded-full transition-all duration-300 ${(formData.family_health_history || []).includes('No significant history') ? 'bg-blue-500/20 text-blue-400' : 'bg-white/5 text-slate-600'}`}>
                    <Heart className="w-6 h-6" />
                </div>
                <div className={`p-3 rounded-full transition-all duration-300 ${formData.lifestyle === 'Very Active' ? 'bg-brand-accent/20 text-brand-accent' : 'bg-white/5 text-slate-600'}`}>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                    <label className="block text-sm font-semibold text-slate-300 ml-1 flex items-center gap-2">
                        <Stethoscope className="w-4 h-4 text-blue-400" /> Family Health History
                    </label>
